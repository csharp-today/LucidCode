...
namespace LucidCode
{
    public static partial class LucidTestExtensions
    {
        ...
        public static async Task<AssertManager<TResult>> ActAsync<TActParameter, TResult>(this Task<ActManager<TActParameter>> manager, Func<TActParameter, TResult> actFunc) =>
        ...
        /// <returns>Manager for Assert step</returns>
        public static async Task<LightAssertManager> ActAsync<TActParameter>(this Task<ActManager<TActParameter>> manager, Action<TActParameter> actFunc) =>
            (await manager).Act(actFunc);
    }
}
